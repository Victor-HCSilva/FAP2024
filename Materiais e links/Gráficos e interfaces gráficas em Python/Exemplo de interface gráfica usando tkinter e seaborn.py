...
def update_plot(plot_type):
    ...
    if plot_type == 'Histogram':
        ...
        if 'Class' in df.columns:
            # Selecionar apenas algumas variáveis para o pairplot
            variables = df.select_dtypes(include='number').columns
            if len(variables) > 1:
                sns.pairplot(df, vars=variables[:4], hue='Class')  # Usar no máximo 4 variáveis para visualização
            else:
...
# Opções de gráficos
plot_types = ['Histogram', 'Boxplot', 'Barplot', 'Scatterplot', 'Pairplot']
